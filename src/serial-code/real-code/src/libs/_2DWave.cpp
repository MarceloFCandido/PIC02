...
_2DWave::_2DWave (double Lx,
    ...
    this->dy = Ly / (Ny - 1);               // y axis's interval
    this->dt = this->dy / 10.;              // Time step 
    this->Ot = (int) ceil(tMax / this->dt); // Number of instants in the time
...
double _2DWave::evaluateFXYT(double x, double y, double t) {
    ...
    */
    ...
    if( tTerm > 0.1*this->tMax ) return 0;
...
mat _2DWave::getVelocitiesMatrix( vector<interface> interfaces,
    ...
    for (int i = 0; i < this->Mx; i++) {
        ...
        x = i * this->dx;   // Step in the abscissas

        for (int j = 0, k = 0; j < this->Ny; j++, k = 0) {

            double y;

            y = j * this->dy;   // Step in the ordinates
